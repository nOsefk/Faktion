...
df_audio_nok = df_audio_nok.sample(n=4000,random_state=42)
...
'so','than','to','up','us','we','will','with','you', 'like','more','our', '000', '10',\
'after', 'also', 'annotation', 'around','back','could','even', 'ever', 'would','where',\
'when', 'want', 'very', 'through', 'there', 'then', 'some', 're', 'other','me', 'just',\
'got','going', 'en', 'go','la']
...
ocr_tfidf_df.drop([col for col, val in ocr_tfidf_df.sum().iteritems() if val < 50], axis=1, inplace=True)             
...
all_text_tfidf_df.drop([col for col, val in all_text_tfidf_df.sum().iteritems() if val < 100], axis=1, inplace=True) 

overlap = ['music', 'new', 'people', 'time', 'world']
all_text_tfidf_df['at_people']= all_text_tfidf_df['people']
all_text_tfidf_df['at_world']= all_text_tfidf_df['world']
all_text_tfidf_df['at_music']= all_text_tfidf_df['music']
all_text_tfidf_df['at_new']= all_text_tfidf_df['new']
all_text_tfidf_df['at_time']= all_text_tfidf_df['time']
...
to_dummies = ['agencies', 'category', 'categoryCode', 'client', 'festivalYear', 'mediaCode', 'mediaDescription', 'section']
...
print(df_final.info())
df_final.to_csv('data_clean_seq.csv')